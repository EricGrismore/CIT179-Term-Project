...
EXEC InventoryLog @GeodeType = 'Ruby'
*/
...
USE myBuisnessOrder;
GO
CREATE FUNCTION fnOwnerRule
    (@FirstName varchar(30))
    RETURNS int
BEGIN
    RETURN (SELECT FirstName FROM [Owner] WHERE FirstName Like @FirstName);
END;

Go 
Select Qty
From OrderDetail
Where Qty = dbo.OwnerRule('1/12/2003');
*/
...
*/
/*
SELECT * INTO InventoryLog 
FROM Inventory;
TRUNCATE TABLE InventoryLog;
ALTER TABLE InventoryLog  ADD  status varchar(20)
*/
/*
CREATE TRIGGER InventoryStock_INSERT ON Inventory AFTER INSERT AS BEGIN SET NOCOUNT ON; 
DECLARE @GeodeID int DECLARE @GeodeType nvarchar(30) DECLARE @Size nvarchar(50) DECLARE @PricePerSize nvarchar(100) 
SELECT @GeodeID =INSERTED.GeodeID, @GeodeType = INSERTED.GeodeType, @Size = INSERTED.Size, @PricePerSize = INSERTED.PricePerSize 
FROM INSERTED INSERT INTO InventoryLog VALUES(@GeodeID,@GeodeType,@Size, @PricePerSize, 'Inserted')
END GO INSERT INTO Inventory VALUES(7,'Ruby', '6"', 4.25);
GO